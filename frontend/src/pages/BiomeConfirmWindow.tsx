...
export default function BiomeConfirmWindow() {
    ...
    const hashRawC = (window.location.hash || "").replace(/^#/, "");
    const hashParamsC = new URLSearchParams(hashRawC.startsWith("?") ? hashRawC.slice(1) : hashRawC);
    const biome = (window as any).__INJECTED_BIOME__ || params.get("biome") || hashParamsC.get("biome") || "UNKNOWN";
    ...
    const handleConfirm = async () => {
        ...
        try {
            if (window.pywebview?.api?.confirm_biome_response) {
                await window.pywebview.api.confirm_biome_response("stay");
            }
        } catch (e) {
    ...
    const handleCancel = async () => {
        ...
        try {
            if (window.pywebview?.api?.confirm_biome_response) {
                await window.pywebview.api.confirm_biome_response("return");
            }
        } catch (e) {
            console.error("confirm_biome_response error:", e);
        }
