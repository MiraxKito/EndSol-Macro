...
interface RecorderWindowProps {
    initialMode?: "obby" | "potion" | "custom";
    onClose?: () => void;
}
...
export default function RecorderWindow({ initialMode, onClose }: RecorderWindowProps = {}) {
    ...
    // Mode: "obby" (default), "potion" or "custom" (Custom Paths tab)
    const [mode, setMode] = useState<"obby" | "potion" | "custom">(initialMode || "obby");
    ...
    const handleStart = async () => {
        ...
        try {
            if (window.pywebview && window.pywebview.api) {
                const result = mode === "potion"
                    ? await window.pywebview.api.start_macro_recording_potion(potionName.trim())
                    : await window.pywebview.api.start_macro_recording();
                if (result && result.ok === false) {
                    setStatusMsg(`Error: ${result.error || "Recorder could not start"}`);
                    return;
                }
                setRecording(true);
                setElapsed(0);
        ...
        try {
            if (window.pywebview && window.pywebview.api) {
                if (mode === "potion") {
                    ...
                    // Custom Paths flow: keep events in memory — the user then
                    // names the path and assigns a feature on the main panel.
                    const res = await window.pywebview.api.stop_macro_recording_custom();
                    setRecording(false);
                    if (res && res.ok) {
                        setStatusMsg(`Stopped! ${res.events} events captured. Now save it in the Custom Paths tab.`);
                    } else {
                        setStatusMsg(`Error: ${res?.message || res?.error || "Stop failed"}`);
                    }
                } else {
    ...
    return (
        <div style={{
            ...
            <h3 style={{ margin: "0 0 10px 0", color: "var(--accent)" }}>
                {mode === "potion" ? "Potion Recorder" : mode === "custom" ? "Custom Path Recorder" : "Obby Recorder"}
            </h3>
                    ...
                    <>
                        ...
                        {(mode === "obby" || mode === "custom") && (
                            <button
                                ...
                                title="Opens and closes the Collection and tilts the camera up — same sequence the macro runs before walking"
                            >
                                Align Camera
                            </button>
                        )}
            ...
            <div style={{ marginTop: "10px", fontSize: "10px", color: "gray", textAlign: "center", maxWidth: "330px" }}>
                {mode === "potion"
                    ? "Make sure to save the record file name as same as the ingame potion name so macro can find the potion and do auto craft for you!"
                    : mode === "custom"
                        ? "Press Align Camera first (same camera setup the macro uses), then record the walk from the spawn point to the target. Stop here and save the recording in the Custom Paths tab (name it and pick a feature)."
                        : "Obby Recorder"}
            </div>
        </div>
    );
