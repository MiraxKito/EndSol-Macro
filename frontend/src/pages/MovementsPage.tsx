            ...
            <div className="card">
                ...
                <div>
                    <ToggleSwitch
                        ...
                        checked={config.enable_obby_path || false}
                        onChange={async (val) => {
                            if (val) {
                                try {
                                    if (window.pywebview?.api) {
                                        ...
                                        if (!hasPath) {
                                            alert("No obby path found!\n\nRecord one via the Recorder (open it from the Custom Paths page), then assign the recording to the obby feature.");
                                            return;
                                        }
                                    }
                                } catch (e) {
                            ...
                            updateConfig("enable_obby_path", val);
                        }}
                    />

                    {config.enable_obby_path && (
                        <div className="form-group" style={{ marginTop: "15px", marginLeft: "10px" }}>
                            ...
                            <input
                                ...
                                type="number"
                                min="1"
                                value={config.obby_claim_interval || "15"}
                                onChange={(e) => updateConfig("obby_claim_interval", e.target.value)}
                                style={{ width: "80px" }}
                            />
                        </div>
                    )}
