...
export default function StatusPage() {
    ...
    useEffect(() => {
        ...
        const fetchDiagnostics = async () => {
            try {
                ...
                if (api?.get_runtime_diagnostics && isMounted) {
                    setDiagnostics(await api.get_runtime_diagnostics());
                }
            } catch (err) {
        ...
        return () => { isMounted = false; clearInterval(diagnosticsInterval); };
    }, []);
    ...
    useEffect(() => {
        ...
        const fetchStatus = async () => {
            if (!window.pywebview?.api?.get_active_modules) {
                if (retries < maxRetries) {
                    retries++;
                    setTimeout(fetchStatus, 1000);
                } else if (isMounted) {
                    setStatusData({ modules: {}, incompatibilities: [] });
                }
                return;
            }
            try {
                const data = await window.pywebview.api.get_active_modules() as ActiveModulesResponse;
                if (isMounted) {
                    setStatusData(data);
                }
            } catch (err) {
    ...
    useEffect(() => {
        ...
        const fetchLogs = async () => {
            if (!window.pywebview?.api?.get_macro_logs) {
                if (retries < maxRetries) {
                    retries++;
            ...
            try {
                const data = await window.pywebview.api.get_macro_logs() as { lines: string[] };
    ...
    if (!statusData) {
        return <div style={{ padding: "20px", color: "var(--text-secondary)" }}>Loading status information...</div>;
    }
    
    // Ensure we have valid data structure
    const modules = statusData.modules || {};
    const incompatibilities = statusData.incompatibilities || [];
            ...
            <div className="card">
                <div className="card-header">
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <p>Read-only checks for Roblox, logs, and calibration state</p>
                    </div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(210px, 1fr))", gap: "8px" }}>
                    <div className="form-hint">Roblox process: {diagnostics?.roblox?.processes?.length ? `${diagnostics.roblox.processes.length} found` : "not found"}</div>
                    <div className="form-hint">Roblox window: {diagnostics?.roblox?.window_found ? "found" : "not found"}</div>
                    <div className="form-hint">Latest log: {diagnostics?.logs?.latest?.name || "not found"}</div>
