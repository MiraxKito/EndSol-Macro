...
export function ConfigProvider({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        ...
        (window as any).onConfigUpdated = () => {
            console.log("Config updated remotely, refreshing values...");
            loadConfig();
        };
        (window as any).onStatsUpdated = () => {
            // Refresh shared values in place; this does not reload the page.
            loadConfig();
        };

        return () => {
            delete (window as any).onConfigUpdated;
            delete (window as any).onStatsUpdated;
        };
    }, []);
    ...
    const loadConfig = async () => {
        try {
            ...
            const data = await window.pywebview.api.get_config() as AppConfig;
            ...
            try {
                if (window.pywebview?.api?.get_biome_data) {
                    const colors = await window.pywebview.api.get_biome_data() as BiomeColorMap;
