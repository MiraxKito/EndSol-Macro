...
def _run_recorded_events(
    *,
    ...
    # Absolute scheduling: every event fires at start + recorded t (scaled for
    # the non-VIP walk stretch). The old version slept the RELATIVE gap before
    # each event and never subtracted the autoit call overhead, so the more
    # mouse events a recording had (camera drags), the slower it played back
    # compared to the recording itself. The recorder's own replay already used
    # absolute scheduling and did not have this problem.
    ...
    try:
        for ev in events:
            ...
            if speed_multiplier and speed_multiplier != 1.0 and dt > 0:
            ...
            # Sleep only while AHEAD of the recorded schedule; once execution
            # overhead puts us behind, catch up instead of drifting further.
            ahead = (start + virtual_t) - time.perf_counter()
            if ahead > 0.004 and not sleep_interruptible(ahead, 0.01):
                return False
...
def _run_walk_to_fish_path(
    *,
    ...
    # Custom fishing spot path takes priority: people fishing on the same
    # server can record their own spot from the spawn point and won't get
    # in each other's way. Falls back to the built-in default walk.
    events: list[dict[str, Any]] = []
    path_meta: dict[str, Any] | None = None
    try:
        from .custom_path_manager import load_path_for_feature_meta
        custom_events, path_meta = load_path_for_feature_meta("fishing")
        if custom_events:
            print("[Fishing] Using custom path for the fishing spot.")
    ...
    # Walk-speed stamp: a custom path recorded in the current Non-VIP/VIP
    # mode plays exactly as recorded; a stamped path recorded in the other
    # mode is stretched/compressed to compensate. Legacy custom paths and
    # the bundled default walk (recorded on VIP) keep the old behavior:
    # stretch when Non-VIP mode is on.
    walk_mult = NON_VIP_WALK_SPEED_MULTIPLIER if non_vip else 1.0
    if path_meta is not None:
        try:
            from .custom_path_manager import resolve_walk_multiplier
            eff = resolve_walk_multiplier(path_meta, non_vip)
            if eff is not None:
                walk_mult = eff
        except Exception:
            pass
    return _run_recorded_events(
        ...
        should_continue=should_continue,
        can_run=can_run,
        speed_multiplier=walk_mult * playback_multiplier,
    )
        ...
        while _should_continue():
            ...
            if not was_runnable:
                ...
                if not _run_walk_to_fish_path(
                    ...
                    should_continue=_should_continue,
                    can_run=_can_run,
                    playback_multiplier=_playback_mult,
                    non_vip=_non_vip,
                ):
                    continue
