...
from .coordinates import enable_dpi_awareness
...
class LifecycleMixin:
    def __init__(self):
        ...
        # Read both historical spellings without deleting or rewriting user data.
        raw_webhook = self.config.get("webhook_urls")
        if not raw_webhook:
        ...
        # Memory Match automation
        self.memory_match_enabled_var = ConfigVar(self.config, "memory_match_enabled", False)
        self.memory_match_play_on_fishing_var = ConfigVar(self.config, "memory_match_play_on_fishing", True)
        self.memory_match_cell_padding_var = ConfigVar(self.config, "memory_match_cell_padding", 4)
        # Quest Board automation (self-sufficient quests only)
        self.quest_board_enabled_var = ConfigVar(self.config, "quest_board_enabled", False)
        self.quest_board_check_interval_var = ConfigVar(self.config, "quest_board_check_interval_minutes", "15")
        self.quest_board_auto_accept_var = ConfigVar(self.config, "quest_board_auto_accept", True)
        self.quest_board_play_with_fishing_var = ConfigVar(self.config, "quest_board_play_with_fishing", True)
        ...
        self.selected_potion_file_var = ConfigVar(self.config, "selected_potion_file", "")
        self.potion_file_1_var = ConfigVar(self.config, "potion_file_1", "")
        self.potion_file_2_var = ConfigVar(self.config, "potion_file_2", "")
        self.potion_file_3_var = ConfigVar(self.config, "potion_file_3", "")
        self.potion_last_file_var = ConfigVar(self.config, "potion_last_file", "")
        ...
        # ── Frontend-controlled settings that were missing ConfigVars ──
        self.rare_biome_screenshot_var      = ConfigVar(self.config, "rare_biome_screenshot", False)
        self.auto_update_enabled_var        = ConfigVar(self.config, "auto_update_enabled", True)
        self.enable_glitch_effect_var       = ConfigVar(self.config, "enable_glitch_effect", False)
        self.enable_potion_switching_var    = ConfigVar(self.config, "enable_potion_switching", False)
        self.potion_switch_interval_var     = ConfigVar(self.config, "potion_switch_interval", "60")

        # ── Auto-start on idle ──
        self.auto_start_on_idle_var         = ConfigVar(self.config, "auto_start_on_idle", False)
        self.auto_start_idle_minutes_var    = ConfigVar(self.config, "auto_start_idle_minutes", "15")
        ...
        # Keep this watcher alive even while the macro is stopped; otherwise
        # the setting could never trigger a first automatic start.
        self._start_idle_monitor()

    def _start_idle_monitor(self):
        """Start the Windows idle watcher used by Auto-Start after inactivity.

        The watcher is deliberately independent from detection_running: this
        option must be able to start the macro while the macro is stopped.
        """
        if getattr(self, "_idle_monitor_thread", None) and self._idle_monitor_thread.is_alive():
            return
    ...
    def _system_idle_seconds():
        """Return Windows' global last-input duration, or None if unavailable."""
        if os.name != "nt":
            return None
        try:
            ...
            if not ctypes.windll.user32.GetLastInputInfo(ctypes.byref(info)):
                return None
            # GetLastInputInfo uses a 32-bit tick counter, so unsigned
            # subtraction correctly handles its wraparound.
    ...
    def _idle_monitor_loop(self):
        last_report = 0.0
        while getattr(self, "_idle_monitor_running", False):
            try:
                enabled = bool(self.config.get("auto_start_on_idle", False))
                ...
                # Any new input rearms the feature, allowing a later idle
                ...
                if not self.check_roblox_procs():
                    ...
                    if now - last_report > 60:
                        self.append_log("[Auto-Start] Inactivity threshold reached, but Roblox is not running; macro was not started.")
                        last_report = now
                ...
                try:
                    self.append_log(f"[Auto-Start] Idle monitor error: {e}")
                except Exception:
                    pass
            time.sleep(2.0)
