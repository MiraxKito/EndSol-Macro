...
def _run_single_egg_route(
    *,
    ...
    # Walk-speed stamp: custom egg routes recorded in the current mode play
    # exactly as recorded; legacy custom routes and the bundled defaults
    # (recorded on VIP) keep the old stretch-when-non-VIP behavior.
    walk_part = NON_VIP_WALK_SPEED_MULTIPLIER if non_vip else 1.0
    if path_meta is not None:
        try:
            ...
            pass
    walk_mult = walk_part * custom_mult
...
def run_egg_collect_once(
    *,
    ...
    # Custom paths assigned to egg_route1..3 take priority over the bundled
    # route files (Custom Paths tab). Their meta carries the walk-speed stamp.
    routes: list[tuple[str, list[dict[str, Any]], dict[str, Any] | None]] = []
    try:
        from .custom_path_manager import load_path_for_feature_meta
        for key in ("egg_route1", "egg_route2", "egg_route3"):
            ev, meta = load_path_for_feature_meta(key)
            if ev:
                routes.append((f"{key} (custom)" if meta else key, ev, meta))
    except Exception:
        pass
    ...
    if not routes:
        for rf in route_files:
            ...
            if events:
                routes.append((rf.name, events, None))
    ...
    for i, (route_name, egg_events, path_meta) in enumerate(routes):
        if not should_continue() or not can_run(): return False
