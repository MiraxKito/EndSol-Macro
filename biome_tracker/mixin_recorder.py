...
class RecorderMixin:
    ...
    def start_recording_path(self):
        ...
        # Ignore the Start button event and the short hand-off back to Roblox.
        # Without this, the recorder window's own click becomes the first
        # crafting event and shifts the entire recipe.
        self._record_armed_at = time.monotonic() + 0.35
    ...
    def _on_mouse_event(self, event):
        if not getattr(self, "_is_recording", False):
            return
        if time.monotonic() < getattr(self, "_record_armed_at", 0.0):
            return
    ...
    def stop_recording_path(self, filename, save_dir="recorded_files", save=True):
        """
        Stop the recording.

        When save=False the events are ONLY kept in memory
        (self._recorded_events) and nothing is written to disk — used by the
        Custom Paths flow, where the user explicitly names the recording and
        assigns it to a feature afterwards (save_recording_as_custom_path).
        """
        if not getattr(self, "_is_recording", False):
        ...
        # Remove the recorder-window stop click without deleting the final
        # keyboard event or a legitimate in-game action.  The old loop stopped
        # as soon as a mouse_move was last, so the UI click often survived.
        if self._recorded_events:
            cutoff_t = self._recorded_events[-1]["t"] - 0.5
            kept = []
            for event in self._recorded_events:
                is_mouse = event.get("type") in ("mouse_down", "mouse_up")
                is_left = str(event.get("button", "")).lower() == "left"
                if is_mouse and is_left and event.get("t", 0.0) >= cutoff_t:
                    continue
                kept.append(event)
            self._recorded_events = kept

        if not save:
            print(f"[Recorder] Stopped recording, {len(self._recorded_events)} events kept in memory (not saved to disk).")
            return "OK"
