...
class RemoteMixin:
    ...
    def _remote_bot_thread_func(self):
        try:
            ...
            # Built for discord.py 2.x (app commands / Interaction API).
            # A thin Client subclass keeps the original slash-command surface
            # (/start, /stop, /status, ...) and syncs it with Discord once
            # per login. No message-content or member intents are needed.
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            intents = discord.Intents.default()

            class _RemoteBot(discord.Client):
                def __init__(self, intents):
                    super().__init__(intents=intents)
                    # Plain Client has no built-in command tree — create it
                    # explicitly (unlike commands.Bot, which auto-creates one).
                    self.tree = app_commands.CommandTree(self)

                async def setup_hook(self):
                    try:
                        # Commands are registered PER-GUILD in on_ready (they
                        # appear instantly there). A global tree.sync() here
                        # would ALSO publish the same commands globally, so
                        # every server would show each command TWICE (global +
                        # guild copy). Instead, purge any stale global
                        # commands that older versions registered, then keep
                        # the tree local-only.
                        try:
                            saved = list(self.tree.get_commands(guild=None))
                            # discord.py >= 2.7 made `guild` a REQUIRED
                            # keyword for clear_commands (omit it and the
                            # whole purge is skipped with a TypeError).
                            self.tree.clear_commands(guild=None)
                            await self.tree.sync()  # empty global set -> Discord deletes stale globals
                            for cmd in saved:
                                try:
                                    ...
                                    pass
                            if saved:
                                purged_msg = (f"[Remote] Purged {len(saved)} stale global command(s); "
                                              f"commands are registered per-server from now on.")
                            else:
                                purged_msg = "[Remote] No stale global commands found — registration is per-server only."
                            print(purged_msg)
                            try:
                                self.append_log(purged_msg)
                            except Exception:
                                pass
                        except Exception as e:
                            cleanup_msg = f"[Remote] Global command cleanup skipped: {e}"
                            print(cleanup_msg)
                            try:
                                self.append_log(cleanup_msg)
                            except Exception:
                                pass
                        return True
                    except Exception as e:
                        print(f"[Remote] Failed to prepare command sync: {e}")
                        return False

            bot = _RemoteBot(intents=intents)
            self.remote_bot_obj = bot

            @bot.event
            async def on_ready():
                # Global command sync (setup_hook) can take up to an hour to
                # propagate. Per-guild sync makes the commands appear
                # instantly in every server the bot is a member of.
                guild_count = 0
                try:
                    ...
                    pass
                guild_note = (f"instant-synced in {guild_count} server(s)"
                              if guild_count
                              else "no guilds visible yet — if the bot is in your server, re-invite it with the 'applications.commands' scope")
                success_msg = f"[Remote] Bot online as {bot.user} — slash commands synced, {guild_note} (discord.py {discord.__version__})"
                print(success_msg)
                try:
            ...
            @bot.tree.command(name="start", description="Start the macro (biome detection)")
            async def start_macro(ctx: discord.Interaction):
                try:
            ...
            async def stop_macro(ctx: discord.Interaction):
                try:
                    if not _check_auth(ctx):
                        await _safe_respond(ctx, "Unauthorized", ephemeral=True)
                        return
            ...
            @bot.tree.command(name="status", description="Show current macro status")
            async def status_macro(ctx: discord.Interaction):
                try:
                    if not _check_auth(ctx):
                        await _safe_respond(ctx, "Unauthorized", ephemeral=True)
                        return
                    ...
                    try:
                        ...
                        embed.set_footer(text=f"EndSol Macro {current_version}")
                    except Exception:
                        pass
            ...
            @bot.tree.command(name="close_roblox_and_stop_macro", description="Kill Roblox process AND stop the macro")
            async def close_roblox_and_stop_macro(ctx: discord.Interaction):
                try:
                    if not _check_auth(ctx):
                        await _safe_respond(ctx, "Unauthorized", ephemeral=True)
                        return
            ...
            @bot.tree.command(name="equip_aura", description="Search for and equip an aura by name")
            async def equip_aura(ctx: discord.Interaction, name: str):
                try:
                    if not _check_auth(ctx):
                        await _safe_respond(ctx, "Unauthorized", ephemeral=True)
                        return
            ...
            @bot.tree.command(name="rejoin", description="Close Roblox to force rejoin (requires Auto Reconnect enabled)")
            ...
            @bot.tree.command(name="help", description="Helping u out with remote access features")
            async def help_cmd(ctx: discord.Interaction, page: int = 1):
                try:
            ...
            @bot.tree.command(name="use", description="Use an item remotely")
            async def use(ctx: discord.Interaction, item: str, amount: int = 1):
                try:
                    if not _check_auth(ctx):
                        await _safe_respond(ctx, "Unauthorized", ephemeral=True)
                        return
            ...
            @bot.tree.command(name="check_merchant", description="Use merchant teleporter and check for merchant")
            async def check_merchant(ctx: discord.Interaction):
                try:
                    if not _check_auth(ctx):
                        await _safe_respond(ctx, "Unauthorized", ephemeral=True)
                        return
            ...
            @bot.tree.command(name="screenshot", description="Take current ingame screenshot and send to webhooks")
            @app_commands.describe(target="Type of screenshot to take")
            @app_commands.choices(target=[
                app_commands.Choice(name="Fullscreen", value="full"),
                app_commands.Choice(name="Inventory", value="inventory"),
                app_commands.Choice(name="Aura Storage", value="aura"),
            ])
            async def screenshot(ctx: discord.Interaction, target: str = None):
                try:
                    if not _check_auth(ctx):
                        await _safe_respond(ctx, "Unauthorized", ephemeral=True)
                        return
            ...
            @bot.tree.command(name="reroll_quest", description="Reroll a selected daily quest")
            @app_commands.describe(quest="Quest to reroll")
            @app_commands.choices(quest=[
                app_commands.Choice(name="Quest 1", value="1"),
                app_commands.Choice(name="Quest 2", value="2"),
                app_commands.Choice(name="Quest 3", value="3"),
            ])
            async def reroll_quest(ctx: discord.Interaction, quest: str):
                try:
                    if not _check_auth(ctx):
                        await _safe_respond(ctx, "Unauthorized", ephemeral=True)
                        return
            ...
            try:
                self.set_title_threadsafe(f"EndSol Macro {current_ver} (Remote bot starting)")
            except Exception:
                pass
            try:
                        ...
                        def _inv_action():
                            try:
                                ...
                                try:
                                    ...
                                    pass
                                try:
                                    self.take_inventory_screenshot_now(force=True)
                                except Exception:
                                    pass
                            finally:
                                self._remote_running = False
                        ...
                        def _aura_action():
                            try:
                                ...
                                try:
                                    ...
                                    pass
                                try:
                                    self.take_aura_screenshot_now(force=True)
                                except Exception:
                                    pass
                            finally:
                                self._remote_running = False
