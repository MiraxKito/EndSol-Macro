...
class GuiMixin:
    def init_gui(self):
        ...
        icon_path = os.path.join(os.path.dirname(abslt_path), "assets", "endsol-macro.ico")

        self.root = ttk.Window(themename=selected_theme)
        self.set_title_threadsafe(f"EndSol Macro {current_ver} (Idle)")
        ...
        def show_frame(name):
            ...
            for nm, fr in frames.items():
                try:
                    ...
                    pass
            try:
                self.status_label.config(text=f"Status: Viewing вЂ” {name}")
            except Exception:
                pass
...
# self.check_for_updates()
    ...
    def create_donations_tab(self, frame):
        t1 = "EndSol Macro is 100% free to use. If you want to support further development, you can leave a donation on Boosty (one-time, any amount, fully optional)."
        t2 = "Donations help cover development time. Everyone who donates can ask to be added to the supporters list below."
        link = "https://boosty.to/eds_mirax/donation"
        ...
        url = GITHUB_RAW_BASE + "/assets/appreciation_list.txt"
        try:
    ...
    def create_notice_tab(self, frame):
        ...
        from .config import GITHUB_RAW_BASE
        notice_url = GITHUB_RAW_BASE + "/assets/noticetabcontents.txt"
        try:
            r = safe_get(notice_url, timeout=10)
            r.raise_for_status()
        ...
        discord_link = "https://discord.gg/endsol"
        ...
        def _open_releases(_=None):
            from .config import GITHUB_RELEASES_PAGE
            webbrowser.open_new(GITHUB_RELEASES_PAGE)

        def _check_latest():
            current_version = current_ver
            try:
                from .config import GITHUB_RELEASES_API
                response = safe_get(GITHUB_RELEASES_API, timeout=10)
    ...
    def create_stats_tab(self, frame):
        ...
        category_order = [
            (BIOME_CATEGORY_WEATHER, BIOME_CATEGORY_LABELS[BIOME_CATEGORY_WEATHER]),
            (BIOME_CATEGORY_RARE,    BIOME_CATEGORY_LABELS[BIOME_CATEGORY_RARE]),
            (BIOME_CATEGORY_EVENT,   BIOME_CATEGORY_LABELS[BIOME_CATEGORY_EVENT]),
            (BIOME_CATEGORY_ADMIN,   BIOME_CATEGORY_LABELS[BIOME_CATEGORY_ADMIN]),
            (BIOME_CATEGORY_OTHER,   BIOME_CATEGORY_LABELS[BIOME_CATEGORY_OTHER]),
        ]
        grouped: dict[str, list[str]] = OrderedDict()
        for cat_key, cat_label in category_order:
            grouped[cat_label] = []
        for biome in biomes:
            cat = get_biome_category(biome)
            cat_label = BIOME_CATEGORY_LABELS.get(cat, BIOME_CATEGORY_LABELS[BIOME_CATEGORY_OTHER])
            if cat_label not in grouped:
                grouped[cat_label] = []
            grouped[cat_label].append(biome)

        columns = 5
        current_row = 0

        for cat_label, cat_biomes in grouped.items():
            if not cat_biomes:
                continue
            # Category header
            header = ttk.Label(frame, text=f"── {cat_label} ──", foreground="white",
                               font=("Segoe UI", 9, "bold"))
            header.grid(row=current_row, column=0, columnspan=columns, sticky="w", padx=5, pady=(6, 1))
            current_row += 1

            for i, biome in enumerate(cat_biomes):
                ...
                row_in_cat = i // columns
                col_in_cat = i % columns
                label.grid(row=current_row + row_in_cat, column=col_in_cat, sticky="w", padx=2, pady=1)
                self.stats_labels[biome] = label

            # Move past the rows used by this category
            rows_used = (len(cat_biomes) + columns - 1) // columns
        ...
        self.total_biomes_label.grid(row=current_row, column=0, columnspan=columns, sticky="w", padx=5, pady=5)

        # Running Session
        session_time = self.get_total_session_time()
        self.session_label = ttk.Label(frame, text=f"Running Session: {session_time}")
        self.session_label.grid(row=current_row + 1, column=0, columnspan=columns, sticky="w", padx=5, pady=10)

        # Biome Logs
        logs_frame = ttk.Frame(frame, borderwidth=2, relief="solid")
        logs_frame.grid(row=0, column=6, rowspan=current_row + 2, sticky="nsew", padx=10, pady=2)
    ...
    def create_credit_tab(self, credits_frame):
        ...
        discord_label = ttk.Label(noteab_frame, text="""Join the EndSol Discord server!!!""", foreground="royalblue", cursor="hand2")
        discord_label.configure(font=('Segoe UI', 9, 'underline'))
        discord_label.pack()
        discord_label.bind("<Button-1>", lambda e: webbrowser.open_new("https://discord.gg/endsol"))

        github_label = ttk.Label(noteab_frame, text="""GitHub: EndSol Macro!""", foreground="#03cafc", cursor="hand2")
        github_label.pack()
